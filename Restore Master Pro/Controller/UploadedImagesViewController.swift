//
//  UploadedImagesViewController.swift
//  Restore Master Pro
//
...
class UploadedImagesViewController: UIViewController {
    ...
    func setLayout(){
        ...
        uploadedImagesCollectionView.alwaysBounceVertical = true
        uploadedImagesCollectionView.collectionViewLayout = layout
    }
...
extension  UploadedImagesViewController: UICollectionViewDelegate, UICollectionViewDataSource, CHTCollectionViewDelegateWaterfallLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        let height = Int.random(in: 80 ... 100)
                                ...
                                if urlOfImage == currentClickUrl{
                                    self.ref.child("profile").child(uid).child(key).removeValue(completionBlock: { (error, _) in
                                        DispatchQueue.main.async {
                                            self.getAllImagsFromFirebase()
                                            self.uploadedImagesCollectionView.reloadData()
                                            ProgressHUD.showSuccess("Your Image Delete Successfully..")
                                        }
                                    })
                                }
                            }
...
extension UploadedImagesViewController: LikeImagesProtocol{
